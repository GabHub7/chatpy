...
<head>
  ...
  <style>
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    /* ── Sidebar ── */
    #sidebar{
      position:absolute;height:100%;z-index:100;width:250px;
      background:#1a1a1a;border-right:2px solid #f5c518;
...
<div id="main">
  ...
  <div id="imgPreviewBar">
    ...
    <button id="removeImg" onclick="removeImage()">✕</button>
  </div>

  <div id="inputBar">
    <button id="photoBtn" onclick="document.getElementById('fileInput').click()">📷</button>
...
<script>
  ...
  function save(){localStorage.setItem(STORE,JSON.stringify(sessions));localStorage.setItem(ACTIVE,activeId||'');}
  ...
  function renderSidebar(){
    ...
    if(!ids.length){list.innerHTML='<div style="color:#555;font-size:.8rem;text-align:center;padding:20px">Belum ada riwayat</div>';return;}
    list.innerHTML=ids.map(id=>{
      const s=sessions[id];
      return `<div class="session-item ${id===activeId?'active':''}" onclick="loadSession('${id}')">
  ...
  function renderChat(){
    ...
    if(!activeId||!sessions[activeId]||!sessions[activeId].messages.length){
      chatEl.innerHTML=`<div class="empty-state" id="emptyState"><div class="big">⚡</div><p>Mulai percakapan baru<br/>dengan ChatPy!</p></div>`;
      return;
    }
  ...
  function addBubble(role,text,time,imageUrl,scroll=true){
    ...
    w.innerHTML=inner;chatEl.appendChild(w);
    if(scroll)chatEl.scrollTop=chatEl.scrollHeight;
  }

  function showTyping(){
    const d=document.createElement('div');d.className='bubble ai typing';d.id='typingDots';
  ...
  async function sendMessage(){
    ...
    try{
      const res=await fetch('/chat',{
        ...
        body:JSON.stringify({message:text||'Tolong analisa gambar ini.',history:sessions[activeId].messages,image:imgData})
      });
